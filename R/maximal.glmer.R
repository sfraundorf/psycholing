#' @title Generalized Linear Mixed Effect Model with Maximal Random Effects
#' Structure
#'
#' @description
#' Determine the call for a \code{glmer} model with the maximal random effects
...
#' @param ... additional arguments to \code{\link{glmer}}, such as
#' \code{family}.
#' @return either a model of class \code{\link{merMod}} or a string containing
#' an \code{glmer} function call that could be used to fit that model.
#' @seealso \code{\link{maximal.lmer}} for normal (Gaussian) linear mixed
#' effects models.
#' @keywords models
...
#' @examples
#' maximal.glmer(data=my.dataframe, outcome='RT',
#'              subjects='Subject', items='Items',
#'              ivs=c('SentenceType', 'PrimeType'),
#'              within.subjects=c('SentenceType', 'PrimeType'),
#'              within.items='PrimeType', fit.model=FALSE, family=binomial)
#' @export
#' @importFrom lme4 glmer

maximal.glmer <- function(data, outcome, subjects='Subject', items='Item',
	...
	full.string <- paste0('glmer(', formula.string,
